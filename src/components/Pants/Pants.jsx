...
function Pants() {
  ...
  const [selectedItems, setSelectedItems] = useState(selections.pants || []);
  const [selectedNone, setSelectedNone] = useState(false);

  useEffect(() => {
    axios.get('http://localhost:5000/Pants')
  ...
  const handleContinue = () => {
    setSelections(prev => ({ ...prev, pants: selectedNone ? [] : selectedItems }));
    navigate('/Shoes');
  };

  return (
    <div className="container">
      <h1>Pick Your Pants</h1>
